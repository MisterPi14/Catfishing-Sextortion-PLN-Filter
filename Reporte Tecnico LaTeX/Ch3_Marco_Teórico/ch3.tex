...
Los Modelos de Lenguaje de Gran Escala constituyen el núcleo tecnológico del presente trabajo. Estos modelos, entrenados en vastas cantidades de texto, poseen la capacidad de comprender relaciones semánticas complejas y patrones lingüísticos característicos de diferentes tipos de comunicación \cite{LLMRef}. Se utilizarán modelos como Llama 3, Gemma 2, Mistral y Qwen para evaluar su desempeño en la tarea de clasificación de mensajes categorizados como catfishing, sextorsión e inofensivos.

\subsection{Aprendizaje por Transferencia y Refinamiento mediante Prompting}

El refinamiento de modelos preentrenados mediante técnicas de prompting (zero-shot, one-shot y few-shot) permite adaptar los LLMs a la tarea específica sin reentrenamiento de pesos \cite{PromptEngByIBM}. Esta aproximación de aprendizaje por transferencia reduce significativamente los requerimientos computacionales y acelera la implementación de soluciones en dominios especializados como la detección de fraudes en línea.

\subsection{Síntesis de Datos mediante Generación Sintética}

Se emplean técnicas de generación de datos sintéticos siguiendo las 8 prácticas propuestas por IBM \cite{SynthDataByIbm} para crear conjuntos de entrenamiento y evaluación balanceados. Esto permite aumentar la diversidad de ejemplos de catfishing y sextorsión sin comprometer el anonimato de las víctimas, utilizando LLMs como sintetizadores de contenido.

\subsection{Procesamiento del Lenguaje Natural (NLP)}

Las técnicas de NLP se emplean para el análisis semántico de mensajes de texto, permitiendo la extracción de características lingüísticas relevantes tales como contexto emocional, patrones de manipulación y señales de coerción características de estafas en línea. Este análisis semántico fundamenta la capacidad del sistema para identificar fraudes de manera oportuna.

\subsection{Arquitectura de Sistemas en Tiempo Real}

El proyecto contempla una arquitectura de procesamiento síncrono que permite analizar y clasificar mensajes instantáneamente conforme se intercambian en la aplicación web. Esta característica de tiempo real es crítica para proporcionar alertas preventivas a los usuarios antes de que se materialice el fraude.

\subsection{Evaluación mediante Matrices de Confusión y Métricas de Desempeño}

Se utilizan matrices de confusión, precisión, recall y F1-score para evaluar comparativamente el desempeño de cada modelo bajo diferentes técnicas de prompting, permitiendo una selección informada del modelo óptimo para el entorno de producción.
...
16GB Ram del sistema con frecuencia de 1600 MHz
...
%https://www-geeksforgeeks-org.translate.goog/machine-learning/essential-metrics-for-model-assessment-tp-tn-fp-fn-in-machine-learning/?_x_tr_sl=en&_x_tr_tl=es&_x_tr_hl=es&_x_tr_pto=tc
