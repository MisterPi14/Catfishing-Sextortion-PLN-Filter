...
Los modelos largos de lenguajes son modelos de aprendizaje profundo que son pre entrenados con grandes cantidades de datos, su función es predecir y generar lenguaje humano a partir de la estimación de probabilidades de que un \textit{token} %palabras, sobpalabras o subcadenas o letras 
...
Dentro de estas tecnicas se encuentran el zero-shot, en donde al modelo se le asigna una tarea para la que no fue entrenado explícitamente, few-shot, que proporciona al modelo resultados de muestra para definir las cualidades que debe cumplir el resultado generado, de manera intermedia existe one-shot tecnica caracterizada por incluir una sola muestra. 
En adición se cuenta con la tecnica chain-of-thought, la cual desglosa tareas complejas en un razonamiento paso a paso para mejorar la comprensión y precisión por parte del LLM.
\cite{PromptEngByIBM}
...
%2025/11/24: redactar que es clasificación binaria y como se adapta a multiclase https://www.datacamp.com/es/blog/classification-machine-learning
...
A lo largo del desarrollo del presente proyecto se empleó la librería de scikit learn a traves del modulo Classification report para la ejecución de los cálculos correspondientes a las diversas pruebas de clasificación realizadas a los modelos largos de lenguaje
...
\end{equation}
%2025/11/22: En algunos casos la ponen como (tp + tn) / (tp + tn + fp + fn) en clasificaciones de 2 clases y en casos de mas de 2 clases tp / (tp + tn + fp + fn) pero esto lo hacen por dar una explicacion sencilla, matematicamente la que esta en esta seccion representa lo mismo: "el n de casos exitosos donde ypred y ytrue coinciden entre los n samples contemplados"
...
%2025/11/22: Esta parte de la redacción se omite en este momento dado que se contempla como información adicional Antonio innecesario pendiente de prof de preguntarle a los profesores de la materia
